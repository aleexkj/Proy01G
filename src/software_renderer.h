...
class SoftwareRenderer : public SVGRenderer {
 public:
  ...
  inline void clear_target() {
    ...
    memset(&sample_buffer[0], 255, 4 * sample_w * sample_h);
  }
...
class SoftwareRendererImp : public SoftwareRenderer {
 ...
 private:
  ...
  /* auxiliar para dibujar lineas */
  void raster_line(float x0, float y0, float x1, float y1, bool dir, Color color);

  // Helpers for supersampling //
  void fill_sample( int x, int y, int ix, int iy, Color c );
  void fill_pixel( int x, int y, Color c );

  // Helpers for transformations
  std::stack<Matrix3x3> tmatrix;
